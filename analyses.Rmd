...
#Following the EuroVoc classification https://en.wikipedia.org/wiki/EuroVoc
annotations$continent[grep("Andorra|Austria|Belgium|France|Germany|Ireland|Liechtenstein|Luxembourg|Monaco|Netherlands|Switzerland|UK", annotations$country)]  <- "Western Europe"
...
annotations$continent[grep("PNG", annotations$country)]  <- "Oceania"
...
annotations$continent[grep("USA|Canada", annotations$country)]  <- "North America"
...
dat = WDI(indicator=c('NY.GDP.PCAP.PP.KD','SE.SEC.CUAT.UP.ZS','SE.TER.CUAT.BA.ZS'), country=c(levels(annotations$country_WB)), start=2012, end=2012)

annotations=merge(annotations,dat,by="country",all.x=T)

plot(annotations$Education.Naomi~annotations$SE.SEC.CUAT.UP.ZS)
plot(annotations$Education.Naomi~annotations$SE.TER.CUAT.BA.ZS)
plot(annotations$Education.Naomi~annotations$NY.GDP.PCAP.PP.KD)
