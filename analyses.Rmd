...
ggplot(dens_dat, aes(x = pc.urban, fill = in.childes)) + geom_density(alpha = 0.5)
...
This graph shows "total fertility rate", namely the number of children a woman has over her whole reproductive period.
...
ggplot(annotations, aes(x=jitter(SE.SEC.CUAT.UP.ZS), y=Education.mid)) + 
    geom_errorbar(aes(ymin=Education.min, ymax=Education.max), colour="black", width=.1) 
...
colnames(mytab)<-c("Setting","Number of participants")
kable(mytab)
...
hist(annotations$Proportion.With.Siblings)
...
annotations$Average.number.of.siblings <- as.numeric(as.character(annotations$Average.number.of.siblings)) #hand-checked, and there is nothing weird, so it's ok to do this, even if it complains about values turned into NAs -- this is bc data missing for some

annotations$totalNchildren = annotations$Average.number.of.siblings + 1 #here we add the key child to the count

hist(annotations$Average.number.of.siblings)
```
...
The next plot shows the total number of children in CHILDES corpora (y axis) as a function of that country's average fertility. If samples were representative, most points would be along the diagonal; if small families are oversampled, points would be below the diagonal (observed N children is lower than expected given fertility). What we observed is that large families are oversampled.

```{r}

plot(annotations$totalNchildren~annotations$fertility,xlim=c(1,4),ylim=c(1,4))
lines(c(0,5),c(0,5))

```

## Integrate with childesr

```{r}
read.csv("childes_metrics.csv")->x
dim(x) #4906

# merge(annotations,x,all.x=T,by.x="Corpus_childesr",by.y="corpus_name")->y
# dim(y) # 5044 -- extra lines mean not everything has been matched
# 
# levels(factor(y[is.na(y$target_child_age),"Corpus"]))  #corpus in our list but NOT on childes

merge(annotations,x,by.x="Corpus",by.y="corpus_name")->annotations
annotations$speaker_type=NA
annotations$speaker_type[annotations$speaker_role %in% c("Mother")]<-"Mother"
annotations$speaker_type[annotations$speaker_role %in% c("Adult","Caretaker",
                                                         "Father","Grandfather",
                                                         "Grandmother","Investigator",
                                                         "Relative","Teacher","Visitor","Teenager")]<-"Adult"
annotations$speaker_type[annotations$speaker_role %in% c("Target_Child")]<-"Key_Child"
annotations$speaker_type[annotations$speaker_role %in% c("Boy","Girl","Brother","Sister","Friend","Playmate")]<-"Other_Child"
#de facto removed: Media, Student, Unidentified


```


Now each line is a transcript from one child at one age. The questions we have are:
- get duration, n of children, n sessions, etc from cha, not from human-annotated table
- derive proportion tokens female adults, male adults, other children, 


```{r}

byCorpusSpeaker_type<- annotations %>%
  select(Corpus, speaker_type, token_sum, type_sum, mlu, duration) %>%
     group_by(Corpus, speaker_type) %>%
       summarise(tok = sum(token_sum), typ = sum(type_sum), mlu = weighted.mean(mlu,token_sum),dur = mean(duration)) %>%
  mutate(tok_d = tok/dur, typ_d = typ/dur, .keep = "all")
...
- how many tokens overall as a function of speaker type by continent -- this tells us how much data overall there are for each
- if we group as Western: North America and Western Europe, non-Western the others, look at pattern of tok, typ, mlu by speaker type across these two
- although in some cases v little data: look at pattern of tok, typ, mlu by speaker type by continent
- I don't think it makes sense to look at same by countries with more vs less 50% population finishes HS, or same for rural/urban (by country)
- nor does it make sense to look at same at level of the family - not enough data points for less versus more educated
- nor for household structure
- nor for corpora where the avg n of sib is smaller than 1 (~60) versus higher (~20)
