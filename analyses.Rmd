...
```{r}
p <- ggpairs(ind_all[,c('log_gdp','SE.SEC.CUAT.UP.ZS','SP.URB.TOTL.ZS','log2_fert')], title="Association across vars: all countries",aes(colour = ind_all$western, fill = ind_all$western, alpha = 0.4),columnLabels=c("GDP per capita (log 10)","Proportion completed high school","Proportion urban","Total fertility (log 2)"), 
               lower = list(continuous = wrap("smooth", alpha = 0.3, size=0.7))) +
  theme_bw()
p

pdf("scattermatrix-all.pdf")
p
dev.off()
```
...
```{r}
p2 <- ggpairs(ind[,c('log_gdp','SE.SEC.CUAT.UP.ZS','SP.URB.TOTL.ZS','log2_fert')], title="Association across vars: all countries",aes(colour = ind$western, fill = ind$western, alpha = 0.4),columnLabels=c("GDP per capita (log 10)","Proportion completed high school","Proportion urban","Total fertility (log 2)"), 
               lower = list(continuous = wrap("smooth", alpha = 0.3, size=0.7))) +
  theme_bw()
p2
