...
########################### main functions
get_childes_data <- function( corpus_args) { #gets data from childes
  utterances <- get_utterances(corpus = corpus_args$corpus)
  tokens <- get_tokens(
...
corpus_args <- list(token = "*") #corpus = included_corpora, 
